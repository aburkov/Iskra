...
When the model produces a token (action) $a$ with probability $\pi(a|s)$ in a given state $s$ and that token leads to a reward $R$, we update the model's parameters proportionally to:
