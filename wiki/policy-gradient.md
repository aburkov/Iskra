...
Here, $J(\theta)$ is the **objective** we want to maximize, $r(q, o)$ is the reward obtained by generating the complete output $o$ in response to a prompt $q$, $P(Q)$ is the probability distribution over input queries, where each query $o \in {O}$ belongs to a set $O$ of all possible inputs; $\pi_\theta(O|q)$ is the policy which, in this case, is used as a probability distribution over full output sequences $o \in {O}$ belonging to some set $O$ of all possible output sequences. $𝔼$ denotes the **expectation** or the **expected value** of the total reward our agent would accumulate if it executed policy $\pi_\theta$ forever, such that each time it starts from a random stage $q$ drawn from $P(Q)$ and draws a completion sequence $o$ from $\pi_\theta(O|q)$.
...
This formulation, which leverages the difference between $Q(q,o)$ and $V(q)$ to compute the advantage, is a fundamental component of [actor-critic methods](actor-critic methods). In actor-critic approaches, the policy $\pi_\theta$ (the actor) selects outputs, while a separate mechanism (the critic) estimates $V(q)$. The critic’s evaluation provides the necessary information to calculate the advantage, thereby guiding the policy updates more effectively.
